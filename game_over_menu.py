...
class GameOverMenu(Menu):
    ...
    def display(self,game):
        ...
        score_massage = self.get_font(35).render(f'Your score: {game.score}', True, ('White'))
        score_massage_rect = score_massage.get_rect(center = ((SCREEN_WIDTH/2), SCREEN_HEIGHT/3-65))
        high_score_message = self.get_font(25).render(f'High score: {game.high_score}', True, ('White'))
        ...
        while run:
            ...
            self._screen.blit(game_over_bg, (0, 0))
            self._screen.blit(game_over_message, game_over_message_rect)
            self._screen.blit(score_massage, score_massage_rect)
            self._screen.blit(high_score_message, high_score_message_rect)
            self._screen.blit(player_dead, player_dead_rect)
            # update button
